...
const NativeDiscordBot = require('./discord/DiscordBot.js');

let discordBot = null;
if (process.env.DISCORD_BOT_TOKEN) {
    discordBot = new NativeDiscordBot({
...
if (firstAdmin && firstAdmin.ok && !configuredAdminPassword) {
    console.log('[Nora] First admin created. Set BOTHIVE_ADMIN_PASSWORD before production.');
    console.log(`[Nora] Login: ${FIRST_ADMIN_EMAIL}`);
    console.log(`[Nora] Temporary password: ${FIRST_ADMIN_PASSWORD}`);
}
...
function pushLog(id, line) {
    const s = getBotState(id);
    let clean = String(line).replace(ANSI_RE, '').replace(/\r/g, '');
    clean = clean.replace(/socks5:\/\/(?:([^:]+):([^@]+)@)?([^:\s\/]+)(?::(\d+))?/gi, '[proxy-tunnel-active]');
...
function startBot(state, bot) {
    ...
    if (ownerId) {
        ...
        if (!isAdmin) {
            const userPrefs = workspaces.preferences(ownerId) || {};
            const userTier = userPrefs.tier || 'free';
            const tierCfg = getTierConfig(userTier, userPrefs);

            // 1. Check running bot count limit
            const runningBots = state.bots.filter(b => b.ownerId === ownerId && b.id !== bot.id && getBotState(b.id).proc).length;
            if (runningBots >= tierCfg.maxBots) {
                return {
                    ok: false,
                    reason: `Plan limit reached: On the ${tierCfg.name} plan you can run up to ${tierCfg.maxBots} bot(s) at a time. Upgrade on the Billing page to run more bots.`
                };
            }

            // 2. Free Plan: Enforce Admin Free Proxy
            if (userTier === 'free') {
                const freePool = proxies.listFree();
                if (!freePool.length) {
                    return {
                        ok: false,
                        reason: 'Free tier requires an Admin-designated Free Proxy, but none are currently configured. Please contact the administrator or upgrade to a paid plan.'
                    };
                }
                const isAssignedFree = bot.config.proxy && freePool.some(fp => proxyToUri(fp) === bot.config.proxy || `${fp.host}:${fp.port}` === bot.config.proxy);
                if (!isAssignedFree) {
                    // Auto-assign the first available free proxy
                    bot.config.proxy = proxyToUri(freePool[0]);
                    saveBotsFile(state);
                    broadcastGlobal(viewer => ({ type: 'bot-updated', bot: publicBot(bot, viewer) }), state);
                }
            }
    ...
    pushLog(bot.id, `[system] Connecting to ${bot.config?.host || 'server'}:${bot.config?.port || 25565} as "${bot.config?.username || bot.id}"`);
    if (bot.config?.proxy) {
        pushLog(bot.id, `[system] Secure proxy tunnel engaged 🛡️`);
    }
...
function stopBot(id) {
    ...
    pushLog(id, `[panel] Gracefully stopping bot "${id}"...`);
...
async function handleHttp(req, res, state) {
    ...
    if (p.startsWith('/api/') && ['POST', 'PATCH', 'DELETE'].includes(req.method) && req.headers.origin) {
        try {
            ...
            const isLocal = originHostname === 'localhost' || originHostname === '127.0.0.1' || originHostname === '0.0.0.0';
            const isAllowed = isLocal ||
                              originHostname === reqHostname ||
                              originHost === reqHost ||
                              originHostname.endsWith('.nativelaunch.xyz') ||
                              originHostname === 'nativelaunch.xyz' ||
                              originHostname.endsWith('.ikie-cli.xyz') ||
                              originHostname === 'ikie-cli.xyz';
    ...
    // API access preflight advertises methods and allowed headers
    ...
    if (req.method === 'POST' && p === '/api/register') {
        ...
        if (!email || !password) {
            return json(res, 400, { ok: false, reason: 'Email/username and password are required' });
        }
        if (password.length < 6) {
            return json(res, 400, { ok: false, reason: 'Password must be at least 6 characters' });
        }
        if (users.findByEmail(email)) {
            return json(res, 400, { ok: false, reason: 'An account with this email/username already exists' });
        }
        const created = users.create({
            email,
            password,
            role: 'user',
            permissions: { allBots: false, botIds: [], categories: [] }
        });
        if (!created.ok) return json(res, 400, created);
        // Default to free plan
        workspaces.updatePreferences(created.user.id, { tier: 'free' });
        const token = createSession(created.user);
        const secure = process.env.BOTHIVE_COOKIE_SECURE === '1' ? '; Secure' : '';
        res.writeHead(200, {
            'Set-Cookie': `bm_session=${token}; Path=/; HttpOnly; SameSite=Lax; Max-Age=86400${secure}`,
            'Content-Type': 'application/json'
        });
        return res.end(JSON.stringify({
            ok: true,
            user: { ...publicUser(created.user), preferences: workspaces.preferences(created.user.id) }
        }));
    }

    if (req.method === 'POST' && p === '/api/auth/oauth-sync') {
        const body = await readJson(req);
        const email = String(body.email || '').trim().toLowerCase();
        if (!email) return json(res, 400, { ok: false, reason: 'Email is required' });

        const adminEmails = (process.env.BOTHIVE_ADMIN_EMAIL || 'admin')
            .split(',')
        ...
        const supabaseRole = String(
            body.role ||
            body.user_metadata?.role ||
            body.app_metadata?.role ||
            (body.user_metadata?.is_admin ? 'admin' : '') ||
            (body.app_metadata?.is_admin ? 'admin' : '') ||
            ''
        ).toLowerCase();

        const isAdmin = supabaseRole === 'admin' || adminEmails.includes(email);
        ...
        if (!user) {
            ...
            const created = users.create({
                ...
                permissions: { allBots: isAdmin, botIds: [], categories: [] }
            });
            if (!created.ok) return json(res, 400, created);
            user = users.findById(created.user.id);
            workspaces.updatePreferences(user.id, { tier: isAdmin ? 'unlimited_15' : 'free' });
        } else if (isAdmin && user.role !== 'admin') {
            users.update(user.id, {
                role: 'admin',
                permissions: { allBots: true, botIds: [], categories: [] }
            });
        ...
        return res.end(JSON.stringify({
            ok: true,
            user: { ...publicUser(user), preferences: workspaces.preferences(user.id) }
        }));
    }
    ...
    if (req.method === 'GET' && p === '/api/me') {
        ...
        if (u) {
            const adminEmails = (process.env.BOTHIVE_ADMIN_EMAIL || 'admin')
                ...
                .map(e => e.trim().toLowerCase())
                .filter(Boolean);
            if (adminEmails.includes(String(u.email || '').toLowerCase()) && u.role !== 'admin') {
                users.update(u.id, { role: 'admin', permissions: { allBots: true, botIds: [], categories: [] } });
            }
            const freshUser = users.findById(u.id);
            return json(res, 200, {
                authenticated: true,
                user: { ...publicUser(freshUser), preferences: workspaces.preferences(freshUser.id) }
            });
        }
        return json(res, 200, {
            authenticated: false,
    ...
    if (req.method === 'GET' && p === '/api/users') {
        if (!requireAdmin(req)) return json(res, 403, { ok: false, reason: 'Forbidden' });
        const enriched = users.list().map(u => {
            const prefs = workspaces.preferences(u.id);
            const userBots = state.bots.filter(b => b.ownerId === u.id);
            const runningBots = userBots.filter(b => getBotState(b.id).proc).length;
            return {
                ...u,
    ...
    if (req.method === 'POST' && p === '/api/users') {
        ...
        if (result.ok && body.tier && ['free', 'bronze_3', 'silver_5', 'unlimited_15', 'custom'].includes(body.tier)) {
            workspaces.updatePreferences(result.user.id, { tier: body.tier });
            result.user.tier = body.tier;
        }
        return json(res, result.ok ? 200 : 400, result);
    }
    ...
    if (userRoute) {
        ...
        if (req.method === 'PATCH') {
            const body = await readJson(req);
            if (body.tier && ['free', 'bronze_3', 'silver_5', 'unlimited_15', 'custom'].includes(body.tier)) {
                workspaces.updatePreferences(id, { tier: body.tier });
            }
            ...
            if (result.ok) {
                result.user.tier = workspaces.preferences(id)?.tier || 'free';
            }
            return json(res, result.ok ? 200 : 400, result);
        }
    ...
    // ── Admin Plans & Revenue Management ──────────────────────────────
    if (p === '/api/admin/plans' && req.method === 'GET') {
        ...
        let totalBotsProvisioned = 0;
        let totalProxiesAllocated = 0;
        const tierDistribution = { free: 0, bronze_3: 0, silver_5: 0, unlimited_15: 0, custom: 0 };

        const subscribers = allUsers.map(u => {
            const prefs = workspaces.preferences(u.id);
            const tierId = prefs?.tier || 'free';
            const customLimits = prefs?.customLimits || null;
            const tierCfg = getTierConfig(tierId, prefs);
            const userBots = state.bots.filter(b => b.ownerId === u.id);
            const runningBots = userBots.filter(b => getBotState(b.id).proc).length;
            const ownedProxies = proxies.list().filter(px => px.owner === u.id).length;

            if (tierDistribution[tierId] !== undefined) {
                tierDistribution[tierId]++;
            } else {
                tierDistribution.custom = (tierDistribution.custom || 0) + 1;
            }

            const price = Number(tierCfg.price || 0);
            if (price > 0 || tierId !== 'free') {
                activePaidUsers++;
                totalMRR += price;
            }
            if (tierCfg.maxBots === Infinity) {
                totalBotsProvisioned += 999;
            } else {
                totalBotsProvisioned += Number(tierCfg.maxBots || 1);
            }
            totalProxiesAllocated += Number(tierCfg.maxProxies === Infinity ? 999 : (tierCfg.maxProxies || ownedProxies || 0));
            ...
            return {
                ...
                maxBots: tierCfg.maxBots === Infinity ? 9999 : tierCfg.maxBots,
                maxProxies: tierCfg.maxProxies === Infinity ? 9999 : tierCfg.maxProxies,
                customLimits,
                lastPayment: prefs?.lastPayment || null,
                runningBots,
                totalBots: userBots.length,
                ownedProxies
            };
        ...
        return json(res, 200, {
            ...
            metrics: {
                ...
                totalBotsProvisioned,
                totalProxiesAllocated,
                tierDistribution,
                serverInfo: {
                    ...
                    activeChildProcs: state.bots.filter(b => getBotState(b.id).proc).length,
                    totalBotsManaged: state.bots.length,
                    totalProxiesManaged: proxies.list().length
                }
            },
            subscribers,
            pricingConfig: {
    ...
    const subMatch = p.match(/^\/api\/admin\/users\/([a-f0-9-]+)\/subscription$/);
    if (subMatch && req.method === 'PATCH') {
        if (!requireAdmin(req)) return json(res, 403, { ok: false, reason: 'Forbidden' });
        const targetUserId = subMatch[1];
        const body = await readJson(req);
        const tier = String(body.tier || '').trim();

        if (!['free', 'bronze_3', 'silver_5', 'unlimited_15', 'custom'].includes(tier)) {
            return json(res, 400, { ok: false, reason: 'Invalid tier specified' });
        }

        const patchData = { tier };
        if (tier === 'custom' || body.customLimits || body.maxBots !== undefined) {
            patchData.customLimits = {
                maxBots: Math.max(1, parseInt(body.maxBots || body.customLimits?.maxBots || 1, 10)),
                maxProxies: Math.max(0, parseInt(body.maxProxies || body.customLimits?.maxProxies || 0, 10)),
                price: Number(body.price != null ? body.price : ((parseInt(body.maxBots || 1, 10) * 0.50) + (parseInt(body.maxProxies || 0, 10) * 0.50)))
            };
        }
        if (body.recordPayment) {
            const tierCfg = getTierConfig(tier, { customLimits: patchData.customLimits });
            patchData.lastPayment = {
                orderId: `admin_override_${Date.now()}`,
                planId: tier,
                amount: tierCfg.price,
                paidAt: new Date().toISOString(),
                note: 'Manually updated by Administrator'
            };
        }

        const preferences = workspaces.updatePreferences(targetUserId, patchData);
        return json(res, 200, { ok: true, preferences });
    }
    ...
    if (p === '/api/proxies' || p.startsWith('/api/proxies/')) {
        ...
        if (m) {
            ...
            if (req.method === 'PATCH' && !m[2]) {
                const body = await readJson(req);
                if (!isAdmin && rec.isFree && rec.owner !== user.id) {
                    return json(res, 403, { ok: false, reason: 'Admin Free Proxies cannot be modified by regular users' });
                }
                const patch = {};
                if (typeof body.note === 'string') patch.note = body.note;
                if (isAdmin && body.isFree !== undefined) patch.isFree = !!body.isFree;
            ...
            if (req.method === 'DELETE' && !m[2]) {
                if (!isAdmin && rec.isFree && rec.owner !== user.id) {
                    return json(res, 403, { ok: false, reason: 'Admin Free Proxies cannot be deleted by regular users' });
                }
    ...
    if (req.method === 'POST' && p === '/api/bots') {
        ...
        const userTier = workspaces.preferences(ownerId)?.tier || 'free';
        if (userTier === 'free' && user.role !== 'admin') {
            if (config.proxy) {
                const isFree = proxies.listFree().some(fp => proxyToUri(fp) === config.proxy || `${fp.host}:${fp.port}` === config.proxy);
                if (!isFree) {
                    return json(res, 403, {
                        ok: false,
                        reason: 'Custom private proxies are locked on the Free Starter plan. You can use Admin Free Proxies or upgrade to Bronze ($3), Silver ($5), or Unlimited ($15).'
                    });
                }
            } else {
                const freeList = proxies.listFree();
                if (freeList.length > 0) config.proxy = proxyToUri(freeList[0]);
            }
        } else if (config.proxy && user.role !== 'admin') {
            const known = body.proxyId ? proxies.get(String(body.proxyId)) : proxies.findByUri(config.proxy);
    ...
    if (m) {
        ...
        if (req.method === 'PATCH' && sub === 'config') {
            const body = await readJson(req);
            const userTier = workspaces.preferences(bot.ownerId || user.id)?.tier || 'free';
            ...
            if (body.proxyId !== undefined) {
                ...
                if (pid) {
                    ...
                    if (userTier === 'free' && user.role !== 'admin' && !rec.isFree) {
                        return json(res, 403, {
                            ok: false,
                            reason: 'Custom private proxies are locked on the Free Starter plan. Upgrade your subscription to use custom proxies.'
                        });
                    }
                    body.proxy = proxyToUri(rec);
                } else {
            ...
            // Same rule as create: a tenant may only dial their own proxies or designated free proxies.
            if (body.proxy && user.role !== 'admin') {
                if (userTier === 'free') {
                    const isFree = proxies.listFree().some(fp => proxyToUri(fp) === body.proxy || `${fp.host}:${fp.port}` === body.proxy);
                    if (!isFree) {
                        return json(res, 403, {
                            ok: false,
                            reason: 'Custom private proxies are locked on the Free Starter plan. Upgrade your subscription to use custom proxies.'
...
async function start(port = 3123) {
    ...
    server.listen(port, host, () => {
        console.log('');
        console.log('Native Control Panel');
        console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
