...
class ProxyStore {
     ...
     * legacy rows); regular users see what they own plus any admin-designated free proxies.
     */
    listFor(user) {
        if (!user) return [];
        if (user.role === 'admin') return this.list();
        return this.data.proxies.filter(p => p.owner === user.id || p.isFree === true);
    }

    /**
     * List all proxies designated by admin as free-tier proxies.
     */
    listFree() {
        return this.data.proxies.filter(p => p.isFree === true);
    }

    /** May this account see/use this entry? */
    canAccess(user, rec) {
        if (!user || !rec) return false;
        if (user.role === 'admin') return true;
        return rec.owner === user.id || rec.isFree === true;
    }

    /** May this account edit/delete this entry? */
    canMutate(user, rec) {
        if (!user || !rec) return false;
        if (user.role === 'admin') return true;
        return rec.owner === user.id && !rec.isFree;
    }
    ...
    update(id, patch) {
        ...
        if (patch.isFree !== undefined) p.isFree = !!patch.isFree;
        this._save();
        return p;
    }
