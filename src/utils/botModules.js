...
const MODULE_CATALOG = {
    ...
    discordBridge: {
        ...
        group: 'Integration',
        describe: 'Relays Minecraft chat, alerts, and bot commands directly to your Discord server or webhook',
        mirror: 'discord',
        fields: [
            { key: 'webhookUrl', label: 'Discord Webhook URL', type: 'text', placeholder: 'https://discord.com/api/webhooks/...', info: 'Instant notifications, chat & event relay' },
            { key: 'token', label: 'Bot Token (Optional)', type: 'text', placeholder: 'MTAx...', info: 'Discord bot token for two-way chat & command control' },
            { key: 'channelId', label: 'Channel ID (Optional)', type: 'text', placeholder: '123456789012345678', info: 'Target Discord channel ID' },
            { key: 'guildId', label: 'Guild / Server ID (Optional)', type: 'text', placeholder: '123456789012345678', info: 'Discord server ID' },
        ],
    },
};
...
function registerBotModules(registry, bot) {
    ...
    registry.register({
        ...
        start: async (opts) => {
            if (opts) bot.config.discord = { ...(bot.config.discord || {}), ...opts, enabled: true };
            if (!bot.discordBridge) {
                const { DiscordBridge } = require('../modules/DiscordBridge.js');
                bot.discordBridge = new DiscordBridge(bot.config, (cmd) => bot.handleDiscordCommand(cmd));
            }
            await bot.discordBridge.init();
        },
        stop: () => {
            if (bot.discordBridge) bot.discordBridge.destroy();
        },
    });
