...
const TIER_CONFIG = {
    free: { id: 'free', name: 'Free Starter', maxBots: 1, maxProxies: 0, allowCustomProxy: false, price: 0 },
    bronze_3: { id: 'bronze_3', name: 'Basic Bronze', maxBots: 3, maxProxies: 3, allowCustomProxy: true, price: 2.00 },
    silver_5: { id: 'silver_5', name: 'Advanced Silver', maxBots: 10, maxProxies: 10, allowCustomProxy: true, price: 5.00 },
    unlimited_15: { id: 'unlimited_15', name: 'Unlimited Pro', maxBots: Infinity, maxProxies: Infinity, allowCustomProxy: true, price: 12.00 },
    custom: { id: 'custom', name: 'Custom Fleet', maxBots: 1, maxProxies: 0, allowCustomProxy: true, price: 0.50 }
};

function getTierConfig(tierId, prefs = null) {
    if (tierId === 'custom' || (prefs && prefs.tier === 'custom')) {
        const customLimits = (prefs && prefs.customLimits) || (prefs && prefs.preferences && prefs.preferences.customLimits) || {};
        const maxBots = Math.max(1, Number(customLimits.maxBots || 1));
        const maxProxies = Math.max(0, Number(customLimits.maxProxies || 0));
        const price = Number(customLimits.price != null ? customLimits.price : (maxBots * 0.50 + maxProxies * 0.50));
        return {
            id: 'custom',
            name: `Custom (${maxBots} Bots${maxProxies > 0 ? `, ${maxProxies} Proxies` : ''})`,
            maxBots,
            maxProxies,
            allowCustomProxy: true,
...
class WorkspaceStore {
    ...
    updatePreferences(userId, patch = {}) {
        ...
        if (['free', 'bronze_3', 'silver_5', 'unlimited_15', 'custom'].includes(patch.tier)) next.tier = patch.tier;
        if (patch.customLimits && typeof patch.customLimits === 'object') {
            next.customLimits = {
                maxBots: Math.max(1, parseInt(patch.customLimits.maxBots || 1, 10)),
                maxProxies: Math.max(0, parseInt(patch.customLimits.maxProxies || 0, 10)),
                price: Number(patch.customLimits.price || 0)
            };
        }
        if (patch.lastPayment && typeof patch.lastPayment === 'object') {
            next.lastPayment = { ...patch.lastPayment };
        }
...
module.exports = { WorkspaceStore, DEFAULT_PREFERENCES, TIER_CONFIG, getTierConfig, normalizeScript };
